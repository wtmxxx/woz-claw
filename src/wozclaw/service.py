...
class ChatService:
    ...
    def _respond_with_optional_session_memory(
        ...
        if session_memory.strip():
            merged_context = f"{long_term_context}\n\n[SESSION_MEMORY]\n{session_memory.strip()}"
        else:
    ...
    def chat(
        ...
        previous_session_memory = str(
            previous_session_state.get("session_memory", "")).strip()
        ...
        long_term_context = self.memory_store.build_long_term_prompt_context(
            context)
        session_memory = context.session_memory if use_latest_session_memory else previous_session_memory
        ...
        record_session_memory = approval_request is None and not (
            runtime_agent is not None and getattr(
                runtime_agent, "_agent", None) is not None
        )
    ...
    def resume_after_approval(
        ...
        # Append tool execution result to session_memory immediately
        self.memory_store.append_session_memory_tool_trace(
            user_id,
            session_id,
            "bash_command",
